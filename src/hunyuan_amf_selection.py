...
def _pair_attention(
    ...
    """Frame-to-frame correspondence. q_i/k_j are [H, HW, D]."""
    q = q_i.mean(dim=0)
    k = k_j.mean(dim=0)
...
def _amf_loss(
    ...
    for t in triplet_ids:
        ...
        # Normalized flow acceleration: penalize abrupt inconsistency without
        # automatically preferring zero/static flow.
        accel = (flow_12 - flow_01).square().sum(dim=-1).sqrt()
        speed_01 = flow_01.square().sum(dim=-1).sqrt()
        speed_12 = flow_12.square().sum(dim=-1).sqrt()
        speed = 0.5 * (speed_01 + speed_12)
        loss_t = (accel / (speed + 0.25)).mean()
            ...
            for beam_id, beam in enumerate(beams):
                ...
                if scoring_failed is not None:
                    if baseline_fallback is None:
                        ...
                        fallback_latents = scheduler_step(beam.latents, fallback_scheduler, step_i)
                    else:
                    ...
                    expanded.append(
                        beam.with_update(
                            latents=fallback_latents, scheduler=fallback_scheduler, reward=0.0, info=info
                        )
                    )
                    continue
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="HunyuanVideo generation with Wan-style AMF-QK candidate selection."
    )
